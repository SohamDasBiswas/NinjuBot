...
class Currency(commands.Cog):
    ...
    async def daily(self, ctx):
        ...
        # Atomic check-and-claim — prevents double reward if two bot instances race
        result = get_db().currency.find_one_and_update(
            {
                "key": key,
                "$or": [
                    {"last_daily": {"$exists": False}},
                    {"last_daily": {"$lte": now - cooldown}},
                ]
            },
            {
                "$set": {"key": key, "guild_id": str(ctx.guild.id), "user_id": str(ctx.author.id)},
                "$inc": {"balance": 0},  # ensure field exists; real inc below
            },
            upsert=False,
        ...
        if result is None:
            # Either on cooldown or doc doesn't exist yet — check which
            data = self.bal(ctx)
            remaining = cooldown - (now - data.get("last_daily", 0))
            if remaining > 0:
            ...
            # New user — no doc yet, create it with upsert
            reward = random.randint(100, 300)
            get_db().currency.update_one(
                {"key": key},
                {"$set": {"key": key, "guild_id": str(ctx.guild.id), "user_id": str(ctx.author.id),
                          "last_daily": now, "wins": 0, "losses": 0},
                 "$inc": {"balance": reward}},
                upsert=True
            )
            data = self.bal(ctx)
        else:
            # Won the atomic race — apply reward and set cooldown
            reward = random.randint(100, 300)
            get_db().currency.update_one(
                {"key": key},
